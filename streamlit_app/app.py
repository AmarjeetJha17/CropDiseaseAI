...
if uploaded_file is not None:
    ...
    with col2:
        ...
        # Simple Advisory (Maharashtra-specific)
        if "Healthy" in predicted_class:
            st.info("✅ **Healthy leaf** — No action needed. Keep monitoring.")
        elif "Bacterial" in predicted_class or "Bligh" in predicted_class:
            st.warning("⚠️ Bacterial Leaf Blight detected. Use copper-based spray. Improve drainage.")
        elif "Redrot" in predicted_class or "Rust" in predicted_class:
            st.warning("⚠️ Fungal disease. Remove infected leaves + apply fungicide.")
        else:
            st.warning("⚠️ Disease detected. Recommend consulting local Krishi Vigyan Kendra.")
        
        st.markdown("**Farmer Advisory**")
        st.write("📍 Pune/Maharashtra region — Monitor weather for next 7 days.")
...
# Footer
st.caption("Built with EfficientNet-B0 | Trained on 3,658 real Maharashtra farm images | 98.63% accuracy")